...
export interface PageHeaderProps extends Omit<BoxProps, 'title'> {
  ...
  /** Phone: the single visible action next to the overflow menu. Only used
      when `overflowActions` is provided; pass explicitly since `actions`
      children cannot be split apart. */
 ...
 * wrapping onto two rows on narrow screens. On phone, `overflowActions`
 * swaps the wrapped row for a primary action + ⋮ overflow menu. */
