...
const BookingsPage: React.FC = () => {
  ...
  const normalBalanceScope = summaryLoaded ? t('summary.scopePastCheckout') : t('summary.scopePastCheckoutPage');
  const companyBalanceScope = summaryLoaded
    ? t('summary.scopeCompanyTerms', { months: COMPANY_OUTSTANDING_MONTHS_AFTER_CHECKOUT })
    : t('summary.scopeCompanyTermsPage', { months: COMPANY_OUTSTANDING_MONTHS_AFTER_CHECKOUT });
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, gap: 2, mb: 3, flexDirection: { xs: 'column', md: 'row' } }}>
        <Box>
          <Typography variant="overline" sx={{ color: 'text.secondary', fontWeight: 900, letterSpacing: 2 }}>
            {t('page.frontDesk')} · {formatOperationalDate()}
          </Typography>
