...
export const BookingChannelChip = ({ channel }: { channel: BookingChannelInfo }) => {
  ...
  return (
    <Tooltip title={t('list.onlineBookingTooltip', { channel: channel.name })} arrow>
      <Chip
        size="small"
        icon={<PublicIcon />}
...
export const NightAuditChip = () => {
  const { t } = useTranslation('bookings');
  return (
    <Chip size="small" label={t('list.nightAudit')} variant="outlined" sx={{ height: 22, fontWeight: 900 }} />
  );
};
