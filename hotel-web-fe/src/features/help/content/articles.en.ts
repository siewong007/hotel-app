...
export const ARTICLES_EN: HelpArticle[] = [
  ...
  // ---------------------------------------------------------- rates & promotions
  {
    ...
    keywords: ['promotion', 'voucher', 'discount', 'promo code', 'offer', 'deal', 'campaign'],
    routePath: '/campaigns',
  ...
  },
  {
    ...
    summary: 'Data Transfer moves backups in and out — three export tiers, previews first, and re-authentication before anything privileged or destructive.',
    category: 'settings-data',
    ...
    blocks: [
      { type: 'paragraph', text: 'Data Transfer handles moving hotel data in and out — for backups, restores and migrations. What you can do depends on your data-transfer permissions: viewing the page, exporting, and importing each need their own grant, and confidential or destructive actions also ask you to re-enter your password (and authenticator code, if enabled).' },
      {
        type: 'steps',
        steps: [
          { title: 'Open Data Transfer', body: 'Requires data_transfer:view. The Export tab shows three tiers: Standard (operational data only), Full, and Full System Backup (everything transferable, with relationship metadata).' },
          { title: 'Export', body: 'Standard needs data_transfer:export. Full and Full System Backup include confidential guest, payment and ledger data — they need data_transfer:export_sensitive and a fresh re-authentication.' },
          { title: 'Review the import preview', body: 'Upload a backup file (data_transfer:import). The preview shows what the file contains, warns when it carries sensitive data, and names any extra permission you still need.' },
          { title: 'Confirm import', body: 'Merge adds without touching existing rows; “update” conflict handling needs data_transfer:override. Full restore deletes existing data first and needs data_transfer:restore plus re-authentication. Only proceed on a file you trust.' },
          { title: 'Check Transfer History', body: 'Past imports, exports and step-up attempts are listed with who ran them — the same rows the audit log records.' },
        ],
      },
      ...
      { type: 'callout', tone: 'tip', title: 'Secrets never travel', body: 'Passwords, sessions, tokens and identity-verification evidence are never in any export tier — no permission level unlocks them.' },
    ],
  },
