...
function ageTone(key: string) {
  if (key === 'current') return 'green' as const;
  if (key === '1_30') return 'blue' as const;
  if (key === '31_60' || key === '61_90') return 'amber' as const;
  return 'red' as const;
}
...
export const OutstandingDrawer: React.FC<{ open: boolean; onClose: () => void; model: ReportsModel }> = ({ open, onClose, model }) => {
  ...
  const ageLabel = (a: ReportsModel['ageing'][number]) => tOr(`reports.ageBuckets.${a.key}`, a.bucket);
  return (
    <Drawer open={open} onClose={onClose} icon="wallet" title={t('reports.outstanding.title')}
      sub={t('reports.outstanding.sub', { date: model.todayLabel })}
      foot={<>
        <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{t('reports.outstanding.source')}</span>
        <span className="spacer" />
        <button className="btn sm"><Icon name="download" size={13} /> {t('common:actions.export')}</button>
      </>}>
      <div className="dw-big">
        <div className="dw-big-l">{t('reports.outstanding.total')}</div>
        <div className="dw-big-v"><Money value={total} tone="due" prefix={symbol} /></div>
      </div>

      <div className="dw-sech">{t('reports.outstanding.buckets')}</div>
      <div className="dw-sec">
        <HotelBarChart
          height={Math.max(160, model.ageing.length * 44)}
          layout="horizontal"
          ariaLabel={t('reports.outstanding.bucketsAria')}
          data={model.ageing.map((a) => ({ bucket: ageLabel(a), value: a.value, key: a.key, count: a.count }))}
          keys={['value']}
          indexBy="bucket"
          colors={({ indexValue }) => {
            const tone = AGEING_TONE[model.ageing.find((a) => ageLabel(a) === indexValue)?.key ?? ''];
          ...
          tooltip={({ indexValue, data: d }) => (
            <div>
              ...
              <div>{t('reports.outstanding.shareOfTotal', { pct: total > 0 ? ((Number(d.value) / total) * 100).toFixed(0) : '0' })}</div>
            </div>
          )}
        />
      </div>
      ...
      <div className="dw-tabs">
        <button className={'dw-tab' + (tab === 'guests' ? ' is-on' : '')} onClick={() => setTab('guests')}>
          <Icon name="user" size={14} /> {t('reports.outstanding.guestBalances')} <span className="ct">{model.guestBalances.length}</span>
        </button>
        <button className={'dw-tab' + (tab === 'company' ? ' is-on' : '')} onClick={() => setTab('company')}>
          <Icon name="building" size={14} /> {t('reports.outstanding.companyBalances')} <span className="ct">{model.companyBalances.length}</span>
        </button>
      </div>

      <table className="dw-table">
        <thead><tr><th>{tab === 'guests' ? t('reports.outstanding.guest') : t('reports.outstanding.company')}</th><th>{t('reports.outstanding.colAgeing')}</th><th className="num">{t('reports.outstanding.colBalance')}</th></tr></thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>
              <td>
              ...
              <td><Pill tone={ageTone(r.ageKey)} sm>{r.age}</Pill></td>
...
export const OccupancyDrawer: React.FC<{ open: boolean; onClose: () => void; model: ReportsModel }> = ({ open, onClose, model }) => {
  ...
  return (
    <Drawer open={open} onClose={onClose} icon="bed" title={t('reports.occupancy.title')}
      sub={t('reports.occupancy.sub')}
      foot={<>
        <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{t('reports.occupancy.foot', { pct: fmtPct(model.live.occNow), occupied, rooms: model.periodRooms })}</span>
        <span className="spacer" />
        <button className="btn sm primary"><Icon name="calendar" size={13} /> {t('reports.occupancy.openTimeline')}</button>
      </>}>
      ...
      <div className="dw-sech">{t('reports.occupancy.byRoomType')}</div>
      <div className="dw-sec">
        <HotelBarChart
          height={Math.max(140, model.roomTypes.length * 44)}
          layout="horizontal"
          ariaLabel={t('reports.occupancy.byRoomTypeAria')}
          ...
          tooltip={({ indexValue, data: d }) => (
            <div>
              <strong>{String(indexValue)}</strong>
              <div>{t('reports.occupancy.tooltipOccupied', { pct: fmtPct(Number(d.occ)) })}</div>
      ...
      <div className="dw-sech">{t('reports.occupancy.departuresToday')} <span className="dw-sech-ct">{model.departures.length}</span></div>
      <table className="dw-table">
        <thead><tr><th>{t('reports.colGuest')}</th><th>{t('reports.colRoom')}</th><th className="num">{t('reports.colBalance')}</th></tr></thead>
        <tbody>
          {model.departures.map((d, i) => (
            <tr key={i}>
              <td><div className="dt-name">{d.name}</div><div className="dt-sub">{d.type} · {t('reports.checkoutAt', { time: d.out })}</div></td>
...
const DepartureBalance: React.FC<{ bal: number }> = ({ bal }) => {
  ...
  return bal > 0 ? <Money value={bal} tone="due" prefix={symbol} /> : <Pill tone="green" sm dot={false}>{t('reports.settled')}</Pill>;
};
...
export const RevenueDrawer: React.FC<{ open: boolean; onClose: () => void; metric?: string; model: ReportsModel }> = ({ open, onClose, metric, model }) => {
  ...
  const titleMap: Record<string, [string, IconName]> = {
    roomRev: [t('reports.titles.roomRev'), 'coins'], totalRev: [t('reports.titles.totalRev'), 'coins'],
    adr: [t('reports.titles.adr'), 'gauge'], revpar: [t('reports.titles.revpar'), 'gauge'],
  };
  const [title, ic] = titleMap[metric || ''] || [t('reports.titles.revenue'), 'coins'];
  return (
    <Drawer open={open} onClose={onClose} icon={ic} title={title}
      sub={t('reports.revenue.sub')}
      foot={<>
        <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{t('reports.revenue.earnedNote')}</span>
        <span className="spacer" />
        <button className="btn sm"><Icon name="list" size={13} /> {t('reports.revenue.viewBookings')}</button>
      </>}>
      <div className="dw-info">
        <Icon name="info" size={16} />
        <div>
          {t('reports.pipeline.info1')} <b>{t('reports.pipeline.booked')}</b>
          {t('reports.pipeline.info2')} <b>{t('reports.pipeline.earned')}</b>
          {t('reports.pipeline.info3')} <b>{t('reports.pipeline.collected')}</b>
          {t('reports.pipeline.info4')} <b>{t('reports.pipeline.outstanding')}</b>
          {t('reports.pipeline.info5')} <b>{t('reports.pipeline.earned')}</b>
          {t('reports.pipeline.info6')}
        </div>
      </div>
      ...
      <div className="dw-sech">{t('reports.revenue.byRoomType')}</div>
      <table className="dw-table">
        <thead><tr><th>{t('reports.colType')}</th><th className="num">ADR</th><th className="num">{t('reports.colOcc')}</th><th className="num">{t('reports.colRevenue')}</th></tr></thead>
        <tbody>
          {model.roomTypes.map((r, i) => (
            <tr key={i}>
              <td><div className="dt-name">{r.type}</div><div className="dt-sub">{t('reports.occupancy.tooltipRooms', { count: r.rooms })}</div></td>
...
export const FlowDrawer: React.FC<{ open: boolean; onClose: () => void; mode?: 'arrivals' | 'departures'; model: ReportsModel }> = ({ open, onClose, mode, model }) => {
  ...
  return (
    <Drawer open={open} onClose={onClose} icon={isArr ? 'login' : 'logout'}
      title={isArr ? t('reports.flow.arrivals') : t('reports.flow.departures')}
      sub={t(isArr ? 'reports.flow.subArrivals' : 'reports.flow.subDepartures', { date: model.todayLabel, count: rows.length })}
      foot={<>
        <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{t('reports.flow.liveNote')}</span>
        <span className="spacer" />
        <button className="btn sm primary"><Icon name="arrow-right" size={13} /> {t('reports.flow.openFrontDesk')}</button>
      </>}>
      <table className="dw-table">
        <thead><tr><th>{t('reports.colGuest')}</th><th>{isArr ? t('reports.colEta') : t('reports.colRoom')}</th><th className="num">{isArr ? t('reports.colNights') : t('reports.colBalance')}</th></tr></thead>
        <tbody>
          {rows.map((r, i) => {
            ...
            return (
              <tr key={i}>
                <td>
                  ...
                  <div className="dt-sub">{r.type} · {isArr ? tOr(`bookings:channels.${arr.source}`, formatStatusLabel(arr.source, arr.source)) : t('reports.checkoutAt', { time: dep.out })}</div>
                </td>
                ...
                <td className="num">
                  {isArr ? <span className="mono" style={{ fontWeight: 700 }}>{arr.nights}</span>
                    : (dep.bal > 0 ? <Money value={dep.bal} tone="due" prefix={symbol} /> : <Pill tone="green" sm dot={false}>{t('reports.settled')}</Pill>)}
                </td>
              </tr>
            );
          })}
