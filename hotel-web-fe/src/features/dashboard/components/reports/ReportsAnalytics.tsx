...
const RANGE_OPTIONS: { v: RangeDays; key: string }[] = [
  { v: 7, key: 'range.days7' }, { v: 30, key: 'range.days30' }, { v: 90, key: 'range.days90' },
];
...
const compareCaption = (
  ...
  compare === 'month'
    ? t('reports.compare.vsLastMonth')
    : compare === 'year'
      ? t('reports.compare.vsLastYear')
      : t('reports.compare.vsPrevDays', { days: rangeDays });
  ...
  const inner = (
    <button className="kpi" onClick={locked ? undefined : onClick} data-clickable={!locked}>
      ...
      <div className="kpi-spark">
        <HotelSparkline values={kpi.spark} color={accent} height={34} ariaLabel={t('reports.kpi.trendAria', { label })} />
      </div>
    </button>
  );
  ...
  const headerActions = (
    <div className="ph-actions">
      <span className="role-pill">
        ...
        {canViewFinancials ? t('reports.access.full') : t('reports.access.scoped')}
      </span>
    ...
    <div className="salim-reports" data-density="comfortable">
      ...
      <div className="filters">
        <Seg value={query.rangeDays} onChange={(rangeDays) => setQuery((q) => ({ ...q, rangeDays }))}
          options={RANGE_OPTIONS.map((o) => ({ v: o.v, l: t(`reports.${o.key}`) }))} />
        {!isPhone && (
          <>
        ...
        <div className="cmp">
          <span className="cmp-l">{t('reports.compare.label')}</span>
          <Seg value={query.compare} onChange={(compare) => setQuery((q) => ({ ...q, compare }))} options={[
            { v: 'prev' as CompareMode, l: t('reports.compare.prev') }, { v: 'month' as CompareMode, l: t('reports.compare.month') }, { v: 'year' as CompareMode, l: t('reports.compare.year') },
          ]} />
        </div>
      </div>
        ...
        <>
          ...
          <section className="live">
            ...
            <div className="live-tiles">
              ...
              <LiveTile icon="broom" n={model.live.toClean} label={t('reports.live.roomsToClean')} tone="rose" />
          ...
          <div className="kpis">
            ...
            <KpiCard icon="gauge" label={t('reports.kpi.adr')} kpi={k.adr} kind="adr" unit="RM"
              accent={accent} showDeltas compareCaption={cmpCaption} onClick={() => open({ type: 'revenue', metric: 'adr' })} locked={!canViewFinancials} />
            <KpiCard icon="gauge" label={t('reports.kpi.revpar')} kpi={k.revpar} kind="revpar" unit="RM"
          ...
          <div className="chart-row two">
            {canViewFinancials ? (
              <Panel phoneCollapsible title={t('reports.panels.dailyRevenue')} icon="chart"
                sub={t('reports.panels.dailyRevenueSub', { room: fmtMoneyK(model.roomRev), other: fmtMoneyK(model.otherRev), days: model.periodDays })}
                right={<Legend items={[{ label: t('reports.series.roomRevenue'), color: palette[0] }, { label: t('reports.series.otherRevenue'), color: palette[1] }]} />}>
                <ChartStateGate loading={loading} isEmpty={model.daily.length === 0}>
                  <HotelLineChart
                    height={250}
                    ariaLabel={t('reports.aria.dailyRevenueTrend')}
                    data={[
                      { id: t('reports.series.roomRevenue'), data: model.daily.map((d) => ({ x: d.date, y: d.room })) },
                      { id: t('reports.series.otherRevenue'), data: model.daily.map((d) => ({ x: d.date, y: d.other })) },
                    ]}
                    enableArea
                    areaOpacity={0.14}
                    ...
                    sliceTooltip={({ slice }) => (
                      <div>
                        ...
                        <div>{t('reports.tooltip.occupancy', { value: fmtPct(model.daily[slice.points[0]?.indexInSeries ?? 0]?.occ ?? 0) })}</div>
                      </div>
                    )}
                  />
                </ChartStateGate>
              ...
              <Panel phoneCollapsible title={t('reports.panels.dailyRevenue')} icon="chart" sub={t('reports.panels.revenueAnalytics')}>
                <Locked label={t('reports.financeOnly')}><div style={{ height: 250 }} /></Locked>
              </Panel>
            )}

            {canViewFinancials ? (
              <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.sourceMix')} icon="globe" sub={t('reports.panels.sourceMixSub')}>
                      ...
                      <HotelPieChart
                        ...
                        tooltip={({ datum }) => (
                          <div>
                            ...
                            <div>{t('reports.tooltip.bookings', { count: sourceBookings(String(datum.id)) })}</div>
                          </div>
                        )}
                      />
                      <div className="donut-center">
                        <div className="dc-v">{fmtMoneyK(sourceTotal)}</div>
                        <div className="dc-l">{t('common:field.total')}</div>
                      </div>
                    </div>
              ...
              <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.sourceMix')} icon="globe">
          ...
          <div className="chart-row two">
            <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.occupancyTrend')} icon="percent"
              sub={t('reports.panels.occupancyTrendSub', { avg: fmtPct(k.occupancy.value), rooms: model.periodRooms, days: model.periodDays })}
              right={<Legend items={[{ label: t('reports.series.dailyOccupancy'), color: accent }]} />}>
              <ChartStateGate loading={loading} isEmpty={model.daily.length === 0}>
                <HotelLineChart
                  height={230}
                  ariaLabel={t('reports.aria.occupancyTrend')}
                  data={[{ id: t('reports.series.occupancy'), data: model.daily.map((d) => ({ x: d.date, y: d.occ })) }]}
                  colors={[accent]}
                  enableArea
                  areaOpacity={0.16}
                  ...
                  sliceTooltip={({ slice }) => (
                    <div>
                      <strong>{fmtShortDate(String(slice.points[0]?.data.x))}</strong>
                      <div>{t('reports.tooltip.occupancy', { value: fmtPct(Number(slice.points[0]?.data.y)) })}</div>
                      <div>{t('reports.tooltip.roomsSold', { count: model.daily[slice.points[0]?.indexInSeries ?? 0]?.occRooms ?? 0 })}</div>
                    </div>
                  )}
                />
              </ChartStateGate>
            ...
            {canViewFinancials ? (
              <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.roomTypePerformance')} icon="bed" sub={t('reports.panels.roomTypePerformanceSub')}>
                <ChartStateGate loading={loading} isEmpty={model.roomTypes.length === 0}>
                  <HotelBarChart
                    height={Math.max(140, model.roomTypes.length * 46)}
                    layout="horizontal"
                    ariaLabel={t('reports.aria.roomRevenueByType')}
                    ...
                    tooltip={({ indexValue, data: d }) => (
                      <div>
                        ...
                        <div>{t('reports.tooltip.roomRevenue', { amount: fmtMoney(Number(d.rev)) })}</div>
                        <div>{t('reports.tooltip.occAdr', { pct: fmtPct(Number(d.occ), 0), adr: fmtMoney(Number(d.adr)) })}</div>
              ...
              <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.roomTypePerformance')} icon="bed">
                <Locked label={t('reports.financeOnly')}><div style={{ height: 200 }} /></Locked>
              </Panel>
            )}
          </div>
          ...
          <div className="chart-row thirds">
            {canViewFinancials ? (
              <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.outstandingAgeing')} icon="wallet" clickable onClick={() => open({ type: 'outstanding' })}
                sub={t('reports.panels.drillDown')} right={<Icon name="arrow-up-right" size={14} style={{ color: 'var(--ink-4)' }} />}>
                <ChartStateGate loading={loading} isEmpty={model.ageing.every((a) => a.value === 0)}
                  emptyMessage={t('reports.panels.noOpenInvoices')}>
                  <HotelBarChart
                    height={Math.max(140, model.ageing.length * 40)}
                    layout="horizontal"
                    ariaLabel={t('reports.aria.outstandingAgeing')}
              ...
              <Panel phoneCollapsible collapseOnPhone title={t('reports.panels.outstandingAgeing')} icon="wallet">
                <Locked label={t('reports.financeOnly')}><div style={{ height: 160 }} /></Locked>
              </Panel>
            )}

            <Panel title={t('reports.panels.arrivalsToday')} icon="login" sub={t('reports.panels.expected', { count: model.live.arrivals })}
              right={<button className="link-btn" onClick={() => open({ type: 'flow', mode: 'arrivals' })}>{t('common:actions.viewAll')} <Icon name="chev-right" size={12} /></button>}>
              <MiniList rows={model.arrivals.slice(0, 4).map((r) => ({ name: r.name, sub: r.type + ' · ' + r.source, side: r.eta, sideMono: true }))} />
            </Panel>

            <Panel title={t('reports.panels.departuresToday')} icon="logout" sub={t('reports.panels.expected', { count: model.live.departures })}
              right={<button className="link-btn" onClick={() => open({ type: 'flow', mode: 'departures' })}>{t('common:actions.viewAll')} <Icon name="chev-right" size={12} /></button>}>
              <MiniList rows={model.departures.slice(0, 4).map((r) => ({
                name: r.name, sub: t('bookings:details.roomNumber', { number: r.room }) + ' · ' + r.out,
                side: r.bal > 0 ? fmtMoney(r.bal) : t('status:ledger.settled'), sideTone: r.bal > 0 ? 'due' : 'ok',
              }))} />
            </Panel>
          </div>
