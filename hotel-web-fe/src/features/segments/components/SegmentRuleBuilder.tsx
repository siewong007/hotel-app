...
function ConditionRow({
  ...
  const { t, tOr } = useTranslation('segments');
  ...
  const valueInput = (m: SegmentFieldMeta) => {
    ...
    if (condition.op === 'in' && m.listCapable && m.options) {
      const options = m.options(fieldOptions ?? EMPTY_OPTIONS, tOr);
      return (
        <TextField
          select
      ...
      case 'tier': {
        ...
        return (
          <TextField
            ...
            sx={{ minWidth: 200 }}
            disabled={disabled}
            helperText={options.length === 0 ? t('builder.noRecordedValues') : undefined}
          >
            {options.map((o) => (
              <MenuItem key={o.value} value={o.value}>
                {o.label}
