...
const LoyaltyDashboard: React.FC = () => {
  ...
  const tierName = (level: number): string => {
    const config = getTierConfig(level);
    return tOr(config.nameKey, config.name);
  };
  const categoryLabel = (category: string): string =>
    tOr(`categories.${category}`, formatCategoryLabel(category));
      ...
      <Box sx={{ m: 3 }}>
        <Alert severity="warning" sx={{ mb: 2 }}>
          ...
          <Typography variant="body2" sx={{
            ...
            {t('dashboard.ekycStatus', { status: ekycStatus ? statusLabel(t, 'ekyc', ekycStatus) : t('dashboard.ekycNotStarted') })}
          </Typography>
              ...
              return (
                <Grid key={reward.id} size={{ xs: 12, sm: 6, md: 4 }}>
                  ...
                  >
                    ...
                    >
                      <Chip
                        label={tOr(tierConfig.nameKey, tierConfig.name)}
                        size="small"
                        sx={{
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                          ...
                          {reward.monetary_value && (
                            <Typography
                              ...
                              {t('dashboard.valueSuffix', { symbol: currencySymbol, amount: formatNumber(reward.monetary_value) })}
                            </Typography>
                          )}
                        </Box>
            ...
            <Box>
              {allRewards.map((reward) => (
                ...
                >
                  <MobileCardRow
                    title={reward.name}
                    subtitle={t('dashboard.subtitlePts', { category: categoryLabel(reward.category), points: formatNumber(reward.points_cost) })}
                    meta={t('dashboard.metaStock', {
                      tier: tierName(reward.minimum_tier_level),
                      stock: reward.stock_quantity !== null && reward.stock_quantity !== undefined ? formatNumber(reward.stock_quantity) : '∞',
                    }) + (reward.monetary_value ? t('dashboard.metaValue', { symbol: currencySymbol, amount: formatNumber(reward.monetary_value) }) : '')}
                    footer={
                      <>
                  ...
                  <TableRow key={reward.id}>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        label={categoryLabel(reward.category)}
                        size="small"
                      />
                    </TableCell>
                    ...
                    <TableCell>
                      <Chip
                        label={tierName(reward.minimum_tier_level)}
                        size="small"
                        sx={{
                    ...
                    <TableCell>
                      {reward.stock_quantity !== null && reward.stock_quantity !== undefined ? formatNumber(reward.stock_quantity) : '∞'}
                    </TableCell>
                    <TableCell>
                      {reward.monetary_value ? `${currencySymbol}${formatNumber(reward.monetary_value)}` : '-'}
                    </TableCell>
                    <TableCell>
                      <IconButton size="small" onClick={() => handleEditClick(reward)} color="primary" aria-label={t('dashboard.editRewardAria', { name: reward.name })}>
                        <EditIcon />
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Box>
                  <Typography variant="h5" component="div" sx={{ fontWeight: 700 }}>
                    {t('dashboard.tierMember', { tier: tierName(membership.tier_level) })}
                  </Typography>
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <Box sx={{ textAlign: 'center' }}>
                ...
                <Typography variant="caption" sx={{ opacity: 0.7 }}>
                  {t('dashboard.lifetimePoints', { count: formatNumber(membership.lifetime_points) })}
                </Typography>
              </Box>
            </Grid>
              ...
              {membership.next_tier ? (
                <Box>
                  ...
                  <LinearProgress
                    variant="determinate"
                    value={tierProgress}
                    aria-label={t('dashboard.progressTo', { tier: membership.next_tier.tier_name })}
                    sx={{
                      height: 10,
                      borderRadius: 5,
                  ...
                  <Typography variant="caption" sx={{ opacity: 0.8, mt: 1, display: 'block' }}>
                    {t('dashboard.morePointsNeeded', { count: formatNumber(membership.points_to_next_tier || 0) })}
                  </Typography>
                </Box>
              ) : (
                <Box sx={{ textAlign: 'center' }}>
              ...
              <Grid key={reward.id} size={{ xs: 12, sm: 6, md: 4 }}>
                ...
                >
                  {isLocked && (
                    ...
                    >
                      <Tooltip title={t('dashboard.requiresTier', { tier: tierName(reward.minimum_tier_level) })}>
                        <Chip
                          icon={<LockIcon />}
                          label={tierName(reward.minimum_tier_level)}
                          size="small"
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        {reward.monetary_value && (
                          <Typography variant="caption" sx={{
                            ...
                            {t('dashboard.value', { symbol: currencySymbol, amount: formatNumber(reward.monetary_value) })}
                          </Typography>
                        )}
                      </Box>
      ...
      <TabPanel value={activeTab} index={1}>
        <Grid container spacing={3}>
          <Grid size={{ xs: 12, md: 6 }}>
            <Card>
              <CardContent>
                <Typography variant="h6" component="h2" gutterBottom sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <CardContent>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Typography variant="body2" sx={{
                      ...
                      {t('dashboard.pointsAway', { count: formatNumber(membership.points_to_next_tier || 0) })}
                    </Typography>
                  </Box>
                  <Alert severity="info" icon={<TrophyIcon />}>
                    {t('dashboard.multiplierAlert', { multiplier: membership.next_tier.points_multiplier, tier: membership.next_tier.tier_name })}
                  </Alert>
                </CardContent>
              </Card>
            </Grid>
                    ...
                    return (
                      <Grid key={tier} size={{ xs: 12, sm: 6, md: 3 }}>
                        ...
                        >
                          <CardContent>
                            <Box sx={{ textAlign: 'center' }}>
                              <Typography variant="h2" component="div">{config.icon}</Typography>
                              <Typography variant="h6" component="h3" sx={{ fontWeight: 600, mt: 1 }}>
                                {tOr(config.nameKey, config.name)}
                              </Typography>
                              {isCurrent && (
                                <Chip
                      ...
                      <ListItemText
                        primary={
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Typography variant="body1">
                              {transaction.description || tOr(`dashboard.txTypes.${transaction.transaction_type}`, formatStatusLabel(transaction.transaction_type))}
                            </Typography>
                            <Typography
                              variant="h6"
                              component="div"
                        ...
                        secondary={
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 0.5 }}>
                            ...
                            <Typography variant="caption" sx={{
                              ...
                              {t('dashboard.balance', { amount: formatNumber(transaction.balance_after) })}
                            </Typography>
                          </Box>
                        }
                      />
            ...
            <Box>
              ...
              <Box sx={{ bgcolor: 'background.default', p: 2, borderRadius: 2, mb: 2 }}>
                ...
                <Typography variant="h5" component="div" sx={{ fontWeight: 700, color: 'primary.main' }}>
                  {t('dashboard.pointsCount', { count: formatNumber(selectedReward.points_cost) })}
                </Typography>
                <Typography variant="caption" sx={{
                  color: "text.secondary"
                }}>
                  {t('dashboard.newBalance', { count: formatNumber((membership?.points_balance || 0) - selectedReward.points_cost) })}
                </Typography>
              </Box>
