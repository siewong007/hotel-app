...
const tOr = (key: string, fallback: string) =>
  translateOr(getActiveLocale(), key, fallback);
...
describe('relativeExpiryLabel', () => {
  it('describes missing, past, today, and future expiries', () => {
    expect(relativeExpiryLabel(null, t, NOW)).toBe('No expiry');
    expect(relativeExpiryLabel('2026-09-10T12:00:00Z', t, NOW)).toBe('Expired 3d ago');
    expect(relativeExpiryLabel('2026-09-13T06:00:00Z', t, NOW)).toBe('Expired today');
    expect(relativeExpiryLabel('2026-09-13T18:00:00Z', t, NOW)).toBe('Expires today');
    expect(relativeExpiryLabel('2026-09-16T12:00:00Z', t, NOW)).toBe('Expires in 3d');
  });
});

describe('voucherSourceLabel', () => {
  it('maps known sources and humanizes unknowns', () => {
    expect(voucherSourceLabel('guest_claim', tOr)).toBe('Guest claim');
    expect(voucherSourceLabel('admin_issue', tOr)).toBe('Issued by staff');
    expect(voucherSourceLabel('partner_api', tOr)).toBe('Partner Api');
  });
});
...
describe('promotionClaimIssue', () => {
  ...
  it('returns a reason for unpublished, unopened, closed, and exhausted offers', () => {
    expect(promotionClaimIssue({ ...published, status: 'draft' }, t, NOW)).toBe(
      'Not published',
    );
    expect(
      promotionClaimIssue(
    ...
    expect(promotionClaimIssue(published, t, NOW)).toBeNull();
  });
});

describe('guestDisplayName', () => {
  it('prefers the guest name and falls back to the id', () => {
    expect(guestDisplayName({ ...baseVoucher, guest_name: 'Aisha' }, t)).toBe(
      'Aisha',
    );
    expect(guestDisplayName({ ...baseVoucher, guest_name: '  ' }, t)).toBe(
      'Guest #9',
    );
    expect(guestDisplayName(baseVoucher, t)).toBe('Guest #9');
    expect(
      guestDisplayName({ ...baseVoucher, guest_id: null }, t),
    ).toBe('—');
  });
});
