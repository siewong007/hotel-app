...
import {
  ...
  FormControlLabel, Grid, InputLabel, MenuItem, Paper, Radio, RadioGroup, Select, Stack, Step, StepLabel,
...
type GuestPaymentChoice = 'pay_at_hotel' | 'paypal_or_card';
...
const PortalBookingPage: React.FC = () => {
  ...
  const [paymentChoice, setPaymentChoice] = useState<GuestPaymentChoice | null>(null);
  ...
  const submitBooking = useCallback(async () => {
    if (!token || !quote || !paymentChoice) return;
  ...
  }, [cleaningPreference, paymentChoice, quote, requestId, runSearch, search, specialRequests, token]);
  ...
  if (confirmation) return <ConfirmationStage confirmation={confirmation} paymentChoice={paymentChoice!} token={token} onStays={() => navigate('/guest-portal?section=stays')} onAnother={() => { setConfirmation(null); setSelectedOffer(null); setQuote(null); setOffers([]); setVoucherId(''); setSpecialRequests(''); setCleaningPreference(false); setPaymentChoice(null); setRequestId(newRequestId()); }} />;
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}>
      ...
      <Typography color="text.secondary" sx={{ mt: 1, mb: 3 }}>Live availability, clear pricing, and your choice of payment before confirmation.</Typography>
      ...
      <Collapse in={Boolean(selectedOffer)} timeout={animationTimeout} unmountOnExit>
        <Box sx={{ mt: 3 }}>{isQuoting || !quote ? <LoadingQuote /> : <ReviewStage quote={quote} search={search} vouchers={vouchers} voucherId={voucherId} selectedOffer={selectedOffer!} selectedVoucher={selectedVoucher} ineligibleVoucherKeys={ineligibleVoucherKeys} specialRequests={specialRequests} cleaningPreference={cleaningPreference} paymentChoice={paymentChoice} isSubmitting={isSubmitting} onVoucher={(value) => void applyVoucher(value)} onRequests={setSpecialRequests} onCleaning={setCleaningPreference} onPaymentChoice={setPaymentChoice} onBack={() => { setSelectedOffer(null); setQuote(null); setPaymentChoice(null); }} onConfirm={() => void submitBooking()} />}</Box>
      </Collapse>
...
function ConfirmationStage({ confirmation, paymentChoice, token, onStays, onAnother }: { confirmation: GuestBookingConfirmation; paymentChoice: GuestPaymentChoice; token: string; onStays: () => void; onAnother: () => void }) {
  const [paymentComplete, setPaymentComplete] = useState(confirmation.status === 'confirmed');
  const isOffline = paymentChoice === 'pay_at_hotel';
  const title = paymentComplete ? 'Booking confirmed' : isOffline ? 'Booking request received' : 'Complete your payment';
  return <Container maxWidth="sm" sx={{ py: { xs: 4, md: 7 } }}><Paper role="status" aria-live="polite" sx={{ p: { xs: 3, sm: 5 }, textAlign: 'center', border: '1px solid', borderColor: 'divider' }}><CheckCircleIcon color={paymentComplete ? 'success' : 'primary'} sx={{ fontSize: 64 }} /><Typography variant="h4" component="h1" sx={{ mt: 2 }}>{title}</Typography><Typography variant="overline" color="text.secondary" sx={{ display: 'block', mt: 2 }}>Booking number</Typography><Typography variant="h3" color="primary.main" sx={{ mt: 0.5, fontVariantNumeric: 'tabular-nums' }}>{confirmation.booking_number}</Typography><Typography sx={{ mt: 3, fontWeight: 700 }}>{confirmation.room_type_name}</Typography><Typography color="text.secondary">{confirmation.check_in_date} to {confirmation.check_out_date}</Typography><Typography variant="h5" sx={{ mt: 2 }}>{money(confirmation.total_amount, confirmation.currency)}</Typography>{isOffline ? <Alert severity="info" sx={{ mt: 3, textAlign: 'left' }}>You chose to pay at the hotel. This booking is pending until the hotel confirms it.</Alert> : <Box sx={{ mt: 3, textAlign: 'left' }}><GuestPaymentPanel mode="session" bookingId={confirmation.booking_id} token={token} amount={confirmation.total_amount} currency={confirmation.currency} showBankTransfer={false} onPaid={() => setPaymentComplete(true)} /></Box>}<Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} justifyContent="center" sx={{ mt: 3 }}><Button variant="outlined" onClick={onStays}>View my stays</Button><Button variant="contained" onClick={onAnother}>Book another stay</Button></Stack></Paper></Container>;
}
