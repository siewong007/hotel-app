...
const stageFile = async (preview: ImportPreview = previewPayload) => {
  svc.uploadBackup.mockResolvedValue({ uploadId: 'u1', bytes: 4, detectedFormat: 'v3' });
...
describe('ImportWizard', () => {
  ...
  it('uploads the file then renders the preview', async () => {
    renderWizard();
    await stageFile();
  ...
  it('runs file → preview → confirm → execute → result', async () => {
    ...
    render(
      <QueryClientProvider
        client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}
      >
        <ImportWizard notify={vi.fn()} onRecord={onRecord} onFinished={vi.fn()} pollIntervalMs={10} />
      </QueryClientProvider>,
    );
    await stageFile();
    ...
    expect(onRecord).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'import', mode: 'merge', jobId: 'job-1', status: 'partial' }),
    );
  });
  ...
  it('restore mode hides conflict policy and warns in the confirm dialog', async () => {
    renderWizard();
    await stageFile();
