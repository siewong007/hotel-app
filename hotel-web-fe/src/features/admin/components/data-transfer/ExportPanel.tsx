      ...
      <Box sx={{ borderTop: `1px solid ${theme.palette.divider}` }}>
          ...
          <AccordionDetails sx={{ px: 2, pt: 0 }}>
            {[...exclusionsByReason.entries()].map(([reason, names]) => (
              <Box key={reason} sx={{ mb: 1.25 }}>
                <Typography variant="body2" sx={{ fontWeight: 700 }}>
                  {exclusionReasonLabel(tOr, reason)}
                </Typography>
        ...
        <TableContainer sx={{ maxHeight: 280 }}>
          <Table size="small" stickyHeader aria-label={t('export.previewScope', { scope })}>
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: 700 }}>{t('export.colEntity')}</TableCell>
        ...
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          {preview && (
            <Typography variant="caption" sx={{ color: 'primary.main', fontWeight: 700 }}>
              {t('export.previewSummary', { records: formatNum(preview.total_records), entities: formatNum(preview.entities.length) })}
            </Typography>
          )}
