...
const RBACManagementPage: React.FC = () => {
  ...
  const submitRole = async () => {
    ...
    try {
      if (roleDialog === 'create') {
        ...
        showSnackbar(t('rbac.roleCreated', { name: created.name }));
        ...
        showSnackbar(t('rbac.roleUpdated', { name: updated.name }));
      }
      setRoleDialog(null);
    } catch (e) {
  ...
  const duplicateRole = async () => {
    ...
    try {
      const created = await createRoleMutation.mutateAsync({
        name: t('rbac.copySuffix', { name: selectedRole.name }),
      ...
      setSelectedId(created.id);
      showSnackbar(t('rbac.roleDuplicated', { name: created.name }));
    } catch (e) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showSnackbar(t('rbac.roleDeleted', { name: deleteTarget.name }));
  ...
  const handleUserCreated = (user: User) => {
    ...
    showSnackbar(t('rbac.userCreated', { name: user.username }));
  };
  const handleUserUpdated = (user: User) => {
    setUsers((prev) => prev.map((u) => (u.id === user.id ? { ...u, ...user } : u)));
    showSnackbar(t('rbac.userUpdated', { name: user.username }));
  };
  const handleUserDeleted = (userId: string) => {
    setUsers((prev) => prev.filter((u) => u.id !== userId));
    showSnackbar(t('rbac.userDeleted'));
  };
  const handleRolesAssigned = (userId: string, roleIds: number[]) => {
    updateUserRoles(userId, roleIds);
    showSnackbar(t('rbac.userRolesUpdated'));
  };
    ...
    return (
      <Box sx={{ p: 3 }}>
        <Alert
          severity="error"
          action={
            <IconButton color="inherit" size="small" onClick={reload} aria-label={t('common:state.retry')}>
              <RefreshIcon />
            </IconButton>
          }
        >
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1480, mx: 'auto', bgcolor: 'var(--hotel-bg)', minHeight: '100%' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 3, flexWrap: 'wrap', mb: 2.25 }}>
        <Box>
          <Box sx={{ fontSize: 11.5, color: T.ink3, fontWeight: 500, display: 'flex', gap: 0.75, mb: 0.75 }}>
            <span>{t('rbac.breadcrumbSettings')}</span><span style={{ color: T.ink4 }}>/</span>
            <span>{t('rbac.breadcrumbAccess')}</span><span style={{ color: T.ink4 }}>/</span>
        ...
        <Tooltip title={t('common:actions.refresh')}>
          <span>
            <IconButton onClick={reload} disabled={loading} aria-label={t('rbac.refreshRoles')} sx={{ border: `1px solid ${T.border}`, borderRadius: '9px' }}>
      ...
      <Box sx={{
        ...
        <PtabBtn id="roles" label={t('rbac.tabRoles')} count={`${roles.length} / ${totalPerms}`} />
        <PtabBtn id="users" label={t('rbac.tabUsers')} count={`${users.length}`} />
      </Box>
      {loading && (
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '300px 1fr' }, gap: 2, alignItems: 'start' }}>
          ...
          <Box sx={{ bgcolor: T.surface, border: `1px solid ${T.border}`, borderRadius: '14px', overflow: 'hidden', position: { md: 'sticky' }, top: 16 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', p: '12px 14px', borderBottom: `1px solid ${T.border}`, background: `linear-gradient(180deg,${T.surface},${T.surface2})` }}>
              <Box sx={{ fontSize: 11, fontWeight: 700, color: T.ink3, letterSpacing: '0.6px', textTransform: 'uppercase' }}>{t('rbac.roles')}</Box>
              <Box sx={{ ml: 'auto', fontSize: 11, fontWeight: 700, color: T.ink3 }}>{t('rbac.rolesConfigured', { count: roles.length })}</Box>
            </Box>
                  ...
                  <Box key={r.id} component="button" onClick={() => setSelectedId(r.id)}
                    ...
                    <Box sx={{ minWidth: 0 }}>
                      ...
                      <Box sx={{ fontSize: 11, color: T.ink3, fontWeight: 500, mt: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {t('rbac.roleMeta', {
                          users: t('rbac.roleUsers', { count: uCount }),
                          granted: cnt,
                          total: totalPerms,
                        })}
                      </Box>
            ...
            <Box sx={{ bgcolor: T.surface, border: `1px solid ${T.border}`, borderRadius: '14px', overflow: 'hidden' }}>
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 2.25, p: '18px 22px', background: `linear-gradient(180deg,${T.surface},${T.surface2})`, borderBottom: `1px solid ${T.border}`, alignItems: 'start' }}>
                    ...
                    <Box sx={{ minWidth: 0 }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.75, mt: 1.5, flexWrap: 'wrap', fontFamily: 'JetBrains Mono, monospace' }}>
                        <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.75 }}>
                          ...
                          <Box sx={{ fontSize: 11, color: T.ink3, fontWeight: 600, fontFamily: 'Inter' }}>{t('rbac.permCountOf', { count: totalPerms })}</Box>
                        </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, alignItems: 'flex-end' }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 0.5 }}>
                        ...
                        <Tooltip title={t('common:actions.duplicate')}><IconButton size="small" onClick={duplicateRole}><CopyIcon sx={{ fontSize: 16 }} /></IconButton></Tooltip>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, p: '12px 22px', borderBottom: `1px solid ${T.border}`, flexWrap: 'wrap' }}>
                <TextField
                  ...
                  placeholder={t('rbac.searchPermissionsFull')}
                ...
                <Button size="small" onClick={() => setHideEmpty((v) => !v)} startIcon={hideEmpty ? <CheckIcon /> : <ModulesIcon />}
                  ...
                  {hideEmpty ? t('rbac.hideEmpty') : t('rbac.showAllModules')}
                </Button>
                      ...
                      <Box key={cat.name} sx={{ border: `1px solid ${open ? T.borderHi : T.border}`, borderRadius: '12px', mt: 1.25, bgcolor: T.surface, overflow: 'hidden' }}>
                        <Box onClick={() => setOpenCats((prev) => { const n = new Set(prev); n.has(cat.name) ? n.delete(cat.name) : n.add(cat.name); return n; })}
                          ...
                          <Box sx={{ minWidth: 0 }}>
                            ...
                            <Box sx={{ fontSize: 11.5, color: T.ink3, fontWeight: 500, mt: '1px' }}>{t('rbac.permCount', { count: cat.permissions.length })}</Box>
                          </Box>
                              ...
                              return (
                                <Box key={p.id}
                                  ...
                                  <Box sx={{ minWidth: 0, display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    <Box sx={{ mt: '4px', display: 'inline-flex', width: 'fit-content', fontSize: 10.5, fontWeight: 700, px: 0.75, py: '1px', borderRadius: '5px', textTransform: 'uppercase', bgcolor: vs.bg, color: vs.fg }}>{verbLabel(t, p.action)}</Box>
                                  </Box>
                  ...
                  return (
                    <>
                      ...
                      {hiddenCats.length > 0 && (
                        <Box sx={{ mt: 1.5, display: 'grid', gridTemplateColumns: '36px 1fr auto', alignItems: 'center', gap: 1.75, p: '12px 14px', border: `1px dashed ${T.borderHi}`, borderRadius: '12px', background: "var(--hotel-surface-sunken)" }}>
                          ...
                          <Box sx={{ fontSize: 12.5, color: T.ink2 }}>
                            <strong style={{ color: T.ink }}>{t('rbac.modulesHidden', { count: hiddenCats.length })}</strong> — {t('rbac.noPermissionsIn')}
                      ...
                      {blocks.length === 0 && hiddenCats.length === 0 && (
                        <Box sx={{ p: 6, textAlign: 'center', color: T.ink3, fontSize: 13 }}>{t('rbac.noMatch')}</Box>
                      )}
                    </>
                  );
                })()}
      ...
      <Dialog open={!!roleDialog} onClose={() => setRoleDialog(null)} maxWidth="xs" fullWidth>
        <DialogTitle>{roleDialog === 'create' ? t('rbac.newRole') : t('rbac.renameRole')}</DialogTitle>
        <DialogContent>
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} maxWidth="xs" fullWidth>
        <DialogTitle>{t('rbac.deleteRole')}</DialogTitle>
        <DialogContent>
          <Typography>
            {t('rbac.deleteRoleWarning', { name: deleteTarget?.name })}
          </Typography>
        </DialogContent>
        <DialogActions>
