...
const CustomerLedgerPage: React.FC = () => {
  ...
  const handleUpdateCompany = async () => {
    if (!editingCompany || !companyEditForm.company_name.trim()) {
      showSnackbar(t('ledger.validation.companyNameRequired'), 'warning');
      return;
    }
  ...
  const handleCreateLedger = async (skipDuplicateCheck = false) => {
    ...
    try {
      ...
      setError(err instanceof Error && err.message ? err.message : t('ledger.errors.createFailed'));
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleUpdateLedger = async () => {
    ...
    try {
      ...
      showSnackbar(bookingRoomRateOverride !== undefined
        ? t('ledger.entryAndRateUpdated')
        : t('ledger.entryUpdated'));
      ...
      setError(err instanceof Error && err.message ? err.message : t('ledger.errors.updateFailed'));
    } finally {
      setUpdating(false);
    }
  };
    ...
    try {
      ...
      if (receiptNumber && !retainedAttempt) {
        ...
        try {
          ...
          console.error('Failed to verify receipt number:', error);
          showSnackbar(t('ledger.errors.receiptVerifyFailed'), 'error');
          return;
        }
        ...
        if (receiptExists) {
          showSnackbar(t('ledger.validation.receiptExists'), 'warning');
          return;
        }
      }
      ...
      setError(err instanceof Error && err.message ? err.message : t('ledger.errors.paymentFailed'));
    } finally {
  ...
  const handleConfirmVoidLedger = async () => {
    ...
    try {
      ...
      setError(err instanceof Error && err.message ? err.message : t('ledger.errors.voidFailed'));
    } finally {
      setVoiding(false);
    }
  };
