    ...
    () =>
      (historyQuery.data?.entries ?? []).map((row) =>
        mapServerHistoryEntry(row, {
          ...
          entities: (count) => t('history.entities', { count }),
          unknownUser: t('history.systemUser'),
        }),
      ),
    [historyQuery.data, t],
  );
        ...
        >
          {canExport && (
            <ToggleButton value="export">
              <DownloadIcon sx={{ fontSize: 18, mr: 0.75 }} /> {t('common:actions.export')}
            </ToggleButton>
          )}
          {canImport && (
            <ToggleButton value="import">
              <UploadIcon sx={{ fontSize: 18, mr: 0.75 }} /> {t('common:actions.import')}
            </ToggleButton>
          )}
          <ToggleButton value="history">
            <HistoryIcon sx={{ fontSize: 18, mr: 0.75 }} /> {t('page.tabHistory')}
          </ToggleButton>
        </ToggleButtonGroup>
      )}
