...
const GuestQuickEdit: React.FC<{ guest: Guest; onSaved: () => Promise<void> | void }> = ({
  ...
  const handleSave = async () => {
    if (!fields.first_name.trim() || !fields.last_name.trim()) {
      setEditError(t('page.namesRequired'));
      return;
    }
    ...
    try {
      ...
      setEditError(errorMessage(err, t('page.updateFailed')));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Stack spacing={1.5}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
        ...
        >
          {saving ? t('common:state.saving') : t('interactions.saveChanges')}
        </Button>
      </Box>
    </Stack>
  );
  ...
  return (
    ...
    >
      <Stack spacing={2.5} sx={{ p: 2.5 }}>
          ...
          <DetailRow
            ...
            value={
              guest.account_username
                ? `${guest.account_username} (${t(guest.account_is_active ? 'drawer.accountActive' : 'drawer.accountDeactivated')})`
                : undefined
            }
          />
        ...
        <Stack spacing={1}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{t('common:field.actions')}</Typography>
