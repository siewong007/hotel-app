...
|---|---|---|---|
| 1 | §11 #2 — off-site backups | Provision a hotel-owned bucket + access list; set `SALIMINN_OFFSITE_REMOTE` (rclone `remote:path`) and `SALIMINN_AGE_RECIPIENTS_FILE` together on the backup unit — both are wired in `deploy/database-backup.sh`. **Declined by the owner 2026-09-25:** backups stay local-only on the AIC host (accepted single-point-of-failure risk), so this gate stays open by decision | `"offsite": true` in `/opt/saliminn/backups/backup-status.json`, plus one remote object pulled and restored |
