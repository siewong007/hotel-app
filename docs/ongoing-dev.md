...
- ~~Manual room-status updates skip validation: `services/rooms.rs::update_room_status_handler` never calls `validate_room_status_transition()` and doesn't auto-create a cleaning task when flipping to `dirty` (unlike the booking-trigger path)~~ — DONE 2026-07-26: handler now runs the same `validate_room_status_transition()` SQL function on the final target (undefined/disallowed → 400) and auto-creates a deduped pending cleaning task on `dirty`/`reserved_dirty`; two matrix rows the handler's own auto-flips need (`occupied→reserved_dirty`, `reserved_dirty→available`) are in the baseline auto-seed and desktop mirror. Verified: `tests/rooms_housekeeping.rs` 6/6 green 2x live, clippy `--all-features --all-targets` clean.
...
- Desktop: Windows/Linux CI packaging; network-fetch pgsql provisioning (today Homebrew-local only); arm or hide the updater (`hotel-desktop/UPDATER.md`); consolidate hand-maintained origin/proxy lists; desktop session persistence (SameSite boundary).
