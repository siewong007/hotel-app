...
// Retained although no route reaches it: tests/booking_service.rs
// (postgres_concurrent_reactivation_allows_only_one_success, run explicitly by
// .github/workflows/ci.yml:126) exercises THIS transactional implementation,
// while the live route path goes through services::bookings::reactivate_booking,
// which uses no transaction at all. Until the live path is made transactional
// and the concurrency test repointed at it, deleting this silently deletes the
// only reactivation concurrency guarantee CI enforces.
