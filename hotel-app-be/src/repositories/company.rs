...
impl CompanyRepository {
    ...
    pub async fn exists_by_id(pool: &DbPool, company_id: i64) -> Result<bool, ApiError> {
        let query = "SELECT EXISTS(SELECT 1 FROM companies WHERE id = $1)";
    ...
    pub async fn exists_by_name(pool: &DbPool, company_name: &str) -> Result<bool, ApiError> {
        let query = "SELECT EXISTS(SELECT 1 FROM companies WHERE LOWER(company_name) = LOWER($1))";
    ...
    pub async fn exists_by_name_except_id(
        ...
        let query = "SELECT EXISTS(SELECT 1 FROM companies WHERE LOWER(company_name) = LOWER($1) AND id != $2)";
