-- Adds 'zh-TW' to the consent_records.locale vocabulary so consent evidence
-- can record that the guest read the notices in Traditional Chinese.
--
-- The platform locale registry (core::i18n::SUPPORTED_LOCALES) already lists
-- zh-TW and consent validation admits it; without this widening a zh-TW
-- consent submission would still fail the check constraint at insert time.
--
...
DECLARE
    ...
    current_definition constant text := $consent_locale_current$CHECK (((locale)::text = ANY (ARRAY[('en'::character varying)::text, ('ms'::character varying)::text, ('zh'::character varying)::text, ('zh-TW'::character varying)::text])))$consent_locale_current$;
    old_definition constant text := $consent_locale_old$CHECK (((locale)::text = ANY (ARRAY[('en'::character varying)::text, ('ms'::character varying)::text, ('zh'::character varying)::text])))$consent_locale_old$;
BEGIN
