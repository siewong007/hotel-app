...
BEGIN
    ...
    IF found_type IS NOT NULL AND
       (found_type <> 'character varying' OR found_length IS DISTINCT FROM 255 OR found_nullable <> 'YES') THEN
        RAISE EXCEPTION 'users.google_subject has incompatible shape: type %, length %, nullable %',
            found_type, found_length, found_nullable;
    END IF;

    found_relation := to_regclass('public.uq_users_google_subject');
    found_index := pg_get_indexdef(found_relation);
    IF found_relation IS NOT NULL AND
       (found_index IS NULL OR found_index <> expected_index) THEN
        RAISE EXCEPTION 'uq_users_google_subject has incompatible definition: %',
