-- Migration: Add 'voided' booking status
-- Voided bookings are excluded from all reports (night audit, analytics, etc.)

-- Drop the existing status constraint and recreate with 'voided' added
...
-- Update the booking sync trigger to treat 'voided' like cancelled/no_show
-- (free the room when a booking is voided)
