...
mod postgres_tests {
    ...
    use hotel_app_be::services::{bookings, ledgers};
    use hotel_app_be::modules::companies::service as companies;
    use rust_decimal::Decimal;
    use sqlx::{PgPool, postgres::PgPoolOptions};

    /// How far the database clock may legitimately lead this process's clock.
    ///
    /// `created_at` is generated by PostgreSQL (`CURRENT_TIMESTAMP`) while the
    /// comparison point comes from `Utc::now()` in the test process. Those are
    /// two different clocks — the dev database runs in a container — so the
    /// database can read marginally ahead and a strict `created_at <= now`
    /// fails intermittently. Observed once as a spurious CI-style failure with
    /// the row only ~35s old and otherwise perfectly valid.
    const DB_CLOCK_LEAD_TOLERANCE_SECS: i64 = 60;

    /// Upper bound on how stale a just-written row's timestamp may be.
    const FRESHLY_WRITTEN_MAX_AGE_SECS: i64 = 300;

    /// Asserts a timestamp decoded from PostgreSQL is a plausible "just now"
    /// instant.
    ///
    /// The window is deliberately asymmetric and generous: it absorbs clock
    /// skew between the database and this process, while staying far tighter
    /// than any genuine decode bug. An epoch value, a `NaiveDateTime` read as
    /// the wrong type, or a timezone-shifted instant is hours-to-decades away
    /// and still fails loudly.
    fn assert_recent_utc(label: &str, value: DateTime<Utc>) {
        let now = Utc::now();
        let age_secs = (now - value).num_seconds();
        assert!(
            (-DB_CLOCK_LEAD_TOLERANCE_SECS..FRESHLY_WRITTEN_MAX_AGE_SECS).contains(&age_secs),
            "{label} should decode as a sane, recent UTC instant, got {value:?} \
             (now {now:?}, age {age_secs}s, allowed {}..{}s)",
            -DB_CLOCK_LEAD_TOLERANCE_SECS,
            FRESHLY_WRITTEN_MAX_AGE_SECS
        );
    }
    ...
    async fn postgres_ledger_payments_reduce_balance_with_exact_decimal_math() {
        ...
        assert_eq!(ledger.status, "pending");
        assert_recent_utc("created_at", ledger.created_at);
