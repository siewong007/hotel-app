...
impl DisposableDatabases {
    async fn from_env() -> Self {
        let configured_url = std::env::var("DATABASE_URL")
            .expect("DATABASE_URL is required for PostgreSQL patch lifecycle tests");
...
fn assert_expected_revisions(revisions: &RevisionSnapshot) {
...
async fn postgres_v1_patches_converge_and_are_idempotent() {
    let mut databases = DisposableDatabases::from_env().await;
    ...
    assert_expected_revisions(&first_revisions);
...
async fn postgres_v1_patch_failures_roll_back() {
    ...
    assert!(!run_patches(&checksum_conflict, None).await.status.success());
    ...
    assert!(!run_patches(&empty, None).await.status.success());
    ...
    assert!(
        !run_patches(&injected_failure, Some(&temporary_catalog.path))
            .await
            .status
            .success()
    );
...
async fn postgres_v1_patch_runners_serialize() {
    ...
    let (first, second) = tokio::join!(run_patches(&upgrade, None), run_patches(&upgrade, None));
    assert_runner_succeeded(&first);
    assert_runner_succeeded(&second);

    let revisions = revision_snapshot(&pool).await;
    assert_expected_revisions(&revisions);
