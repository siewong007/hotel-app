...
- Wrong: assumed renaming backend `/rbac/users` would collide with data.sql's self-validating `expected_route_access_policies` block (2026-07-15 lesson). It does NOT — those rows are FRONTEND nav routes (`('rbac','/rbac','Access Control','config',…)`), so backend API path renames never touch data.sql. Check which layer a policy table describes before pricing a rename as schema-risky.
