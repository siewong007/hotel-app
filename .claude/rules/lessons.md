...
## 2026-07-18 — nav tabs hidden: PG data.sql drifted behind SQLite by 20 route_access_policies; data.sql is now one-time-only (contradicts 2026-07-15 idempotency step)
- Trigger: admin/super_admin saw only 5 nav tabs. FE gate hotel-web-fe/src/navigation/routeRegistry.tsx:412 `canAccessNavigationRoute` returns false when an `accessControlled: true` route has NO matching route_access_policy. Postgres had only 5 policies (housekeeping/support/promotions/communications/online-inventory) while `expected_route_access_policies` (data.sql:240) declares 25 and SQLite (database/sqlite/data.sql:404-423) already seeded the full set — a silent dual-DB divergence.
- Wrong: (a) assuming permissions drive tab visibility — the DB route_access_policies row is the gate, permissions only filter within it; admin having all perms is not enough if the policy row is absent. (b) assuming SQLite and PG seeds stayed in parity. (c) trying to verify by re-applying data.sql to the existing DB "for idempotency" per the 2026-07-15 lesson.
- Right: data.sql now begins (data.sql:16-27) with a one-time guard that RAISEs `data.sql is a one-time V1 installation step` if hotel_schema_revisions has generation=1/version=1 — so it is NOT idempotent and must NOT be re-run against an existing V1 DB. Verify seed changes by applying baseline+data+seed ONCE to a fresh scratch DB (create a throwaway database in the running container, pipe the 3 files via `docker exec -i psql -v ON_ERROR_STOP=1`); for an already-initialized DB, apply only the new statements directly (the INSERT...ON CONFLICT block is safe standalone). PG seed validation RAISEs on any route policy referencing a non-existent permission (data.sql:1081) or role (data.sql:1100), so reference only names present in that DB — SQLite's set references loyalty:*/rooms:write/permissions:read/navigation_* markers that do NOT exist in the PG seed, so PG policies must use base perms (admin/super_admin hold all → still see every tab). `missing_seed_count` (data.sql:1049) is computed but never RAISEd, which is why 5≠25 didn't fail the seed.
- Rule: for "why is a nav tab missing", check route_access_policies has a row for that route.id BEFORE touching permissions. Any route_access_policies seed change must be made in BOTH database/postgres/data.sql AND database/sqlite/data.sql and verified by a fresh scratch apply of each (PG in a scratch DB, SQLite via `sqlite3 scratch.db < baseline < data < seed`) — never by re-running data.sql on a live DB (the one-time guard blocks it). Supersedes the "apply data.sql twice for idempotency" step in the 2026-07-15 route-policy lesson.
...
- Verify: cargo check passed on --all-features AND --features sqlite --no-default-features; no test asserts the SameSite value (grepped). Confirmed backend now emits `SameSite=Lax` via curl to :3030. CANNOT script Safari (computer-use grants browsers read-only) — the click and the restore must be confirmed by the user in Safari.
