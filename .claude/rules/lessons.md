...
- **Nothing in this repo applies a second migration file.** A `0002_*.sql` written to a plan's "forward-only migration" convention was inert — CI, deploy, `sync-desktop-resources.mjs`, and `postgres.rs` all hardcode `0001_v1_baseline.sql`. Before writing any schema change, `grep -rn "<filename>" .github/workflows hotel-desktop/scripts hotel-desktop/src-tauri/src`; if nobody reads it, the SQL is dead however correct it is.
