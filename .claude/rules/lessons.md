...
**The local branch is also not `origin/master`.** Observed 2026-09-05: local `master` sat
`[ahead 2, behind 3]` — the 3 behind were PR #127, which had already shipped anonymous
booking, patch 0009 and `unpaid_hold_scheduler.rs`, while the 2 ahead were another session's
freshly written plan and spec **for that same, already-shipped feature**. A third session's
dirty `guest_booking/models.rs` added a test for `AnonymousBookingContact`, a type absent
from the checked-out tree and therefore uncompilable here.

**Rule:** run `git status --short --branch` (not bare `git status`) at session start and read
the ahead/behind counts. Before writing any plan, spec, or "this is missing" finding, check
`git log --oneline HEAD..origin/master` and `git ls-tree origin/master <path>` — the feature
may already be merged. A local file's absence proves nothing about the repository.
...
- **zsh applies history modifiers inside `"$var:h"`.** `git show "$R:hotel-app-be/x.rs"` with `R=origin/master` expanded to `originotel-app-be/x.rs` — zsh read `:h` as the dirname modifier, stripping `/master`. Quoting does NOT disable it. The error names a path you never typed, which reads like a missing file rather than a broken variable. Use `${R}:path`, or just write the ref literally.
...
- **`ssh` inside a script piped to `bash -s` eats the rest of the script.** A remote-provisioning script whose first command was `sudo ssh <host> 'install -d /opt'` ran that one command and exited 0 with no further output — ssh had consumed the remaining lines from stdin as its own. Silent, and looks like the script simply produced nothing. Use `ssh -n` (or `< /dev/null`) for every ssh call in a script that is itself being fed on stdin; rsync manages its own child ssh and is unaffected.
- **`pkill -f <pattern>` matches the shell running it.** `bash -c 'pkill -f "python3 -m http.server"; ...'` killed its own ssh session (exit 255) because the pattern appeared in that shell's own command line. The cleanup had actually worked; only the reporting died. Match on a bracketed pattern (`pgrep -laf "[h]ttp[.]server"`) or pkill by exact name.
- **macOS `tar -cf - .` piped to a remote `tar -xf -` ships AppleDouble junk.** `._Caddyfile` and friends land alongside the real files and the destination directory inherits the local uid (501). Use `COPYFILE_DISABLE=1`, or push single files with `ssh host 'cat > /path'`, which is cleaner for config anyway.
- **`cargo fmt -- <path>` formats the WHOLE crate.** Everything after `--` is passed to rustfmt as an *option*, not a file filter; cargo still enumerates every target. One such call reformatted 54 unrelated files in this tree — including a test file another session had open — and the noise is not whitespace-only (rustfmt reorders imports and rewraps signatures, so `git diff -w` still shows it). To format one file: `rustfmt --edition 2024 <path>`. If it has already happened, revert by exclusion (`git diff --name-only | grep -v <your files> | tr '\n' '\0' | xargs -0 git checkout --`) and never `checkout` a path that was dirty before you started. *(Updated 2026-09-25: both crates are now rustfmt-clean and CI runs `cargo fmt --check`, so on a committed tree a whole-crate `cargo fmt` rewrites only unformatted uncommitted work — normally just yours. Run it before pushing; the one-file recipe still matters when another session's dirty files share the tree.)*
...
- **`cargo test --all-features` exits 0 with every money test skipped, and RUN COUNT DOES NOT DETECT IT.** *(Corrected 2026-09-15 — this entry previously said "assert the RUN COUNT"; that advice was backwards and is the reason it is called out here.)* The PG-gated files early-return when `DATABASE_URL` is unset, and **libtest counts an early return as a `pass`** — so removing the database makes the total go **UP**, not down. Measured on master (`923c12bfd`), no `DATABASE_URL`: **1,317 passed / 0 failed / 1 ignored across 54 binaries, exit 0**, of which 958 were the `src/` unit tests counted twice (`main.rs` re-declared every module, so lib and bin each reported 479). **Half-superseded 2026-09-19:** `main.rs` now links the lib, so the bin contributes **0** unit tests (measured: `--lib` 509 passed, `--bins` 0). A no-DB total well BELOW the old 1,317 is now expected and is NOT evidence that tests were lost — the skip-counts-as-pass trap this entry is really about is unchanged. The tell is per-suite: `payment_characterization` reports **44 passed in 0.01s** with no database versus a real **29 passed / 2 ignored**. A *higher* number means *less* was tested. Judge by wall-clock + per-suite counts, or run `-- --nocapture` and grep for `skipping`. An exit code is not evidence here, and neither is the total.
- **`cargo check` does not compile `tests/`.** After changing any `pub fn` signature, run `cargo test` or at least `cargo check --tests`.
- **`clippy --all-targets` is NOT a superset of plain clippy.** *(Rewritten 2026-09-19 — the premise changed.)* `main.rs` USED TO re-declare every module instead of using the lib, so the crate compiled twice and `dead_code` applied in full to the bin copy. It now `use`s `hotel_app_be` and the crate compiles ONCE: measured A/B on the same leaf-file edit, `cargo check --all-features` went **8.75s → 5.98s** (median of 3 each), and incremental build+link after touching `main.rs` **22.8s → 12.9s**. **The bin-target `dead_code` signal is therefore GONE, not merely quieter** — every lib module is `pub`, so its items are externally reachable by construction. This was a deliberate trade, taken after confirming plain clippy reported **zero** dead-code findings at the time, so no existing finding was lost; what is lost is future detection. A qualified grep across `tests/` is now the only guard before deleting a suspected-dead item. CI runs `cargo clippy --all-features -- -D warnings` with no `--all-targets`, and that still passes. **`--all-targets` was ALREADY red on committed code** before this change — 9 style errors in `tests/distributed_state.rs` (`manual RangeInclusive::contains`, `assert_eq!` with a literal bool), verified byte-identical with the old and new `main.rs`, so do not attribute it to the thin bin. Copy CI's command string character for character.
...
- **Backend tests can run on this Windows machine — build a scratch database, never point `DATABASE_URL` at the live hotel DB.** `createdb hotel_app_test` on the embedded cluster (`hotel-desktop/src-tauri/pgsql/bin/psql.exe -h 127.0.0.1 -p 5433 -U hotel_admin`, password in `%LOCALAPPDATA%\HotelApp\postgres-password.txt`), then apply the baseline, `seed.sql`, and **every patch listed in `patches/manifest.tsv`** in order; they apply clean on a fresh baseline, so a preflight RAISE means the TARGET database has drifted, not that the patch is wrong. (**Corrected 2026-09-15:** this used to read "patches 0002–0009 … all nine". The 1.2–1.23 lineage was folded into the baseline in `77001d419` and the catalog republished from empty; today it holds exactly **0002 deposit_forfeited** and **0003 guest_relations_phase2**, and the baseline now lives at `database/postgres/migrations/0001_v1_baseline.sql`. Read the manifest, never a remembered patch range.) Export `postgres://hotel_admin:<pw>@127.0.0.1:5433/hotel_app_test` and the integration suites run for real (payment_characterization 29 passed / 2 ignored). Two targets never compile here — `postgres_patch_catalog` and `postgres_patch_lifecycle` use `std::os::unix` — so `cargo test --all-features` always fails on Windows; iterate `--test <name>` and skip those two. `openapi_drift` was previously failing on a clean tree (`/health` vs `/api/health` in `docs/api/openapi.json`); the spec was regenerated with the sanctioned command in 4836a846 and PR #127 merged green, so treat that note as PROBABLY RESOLVED and re-verify before blaming it — either way, verify a failure against a stashed tree before believing it is yours. Drop the scratch DB when done.
...
- **Before "fixing" a rule that rejects valid-looking input, find the test net behind it.** The 409 "Transaction reference was already used with different payment data" looked like over-strict validation on a free-text memo box, and booking-scoping it was written, compiled, and clippy-clean before four concurrency tests turned up (`payment_characterization.rs` ~1672/1749/1822/1930) proving global scope is a deliberate money control: one inbound payment must not be credited to two bookings. The signal was reaching for "delete the tests that disagree". Grep the test tree for the invariant BEFORE changing it, and re-ask the user once its real cost is visible — the first answer was given without it. What was genuinely broken was narrower: the create path deliberately excludes pre-idempotency rows (`idempotency_fingerprint IS NOT NULL`) and the edit path did not, so all 707 historical reference values blocked edits, untested. Cryptic-but-correct guards are better fixed by naming the owner in the message than by loosening the rule.
...
- **ky 2 pre-consumes the error body.** Hooks moved from positional args to one state object (`beforeRequest`/`afterResponse`/`beforeError`), `prefixUrl` became `prefix` (now tolerant of a leading `/`), and `BeforeErrorState` has no `.response` — narrow with `isHTTPError(error)`. The silent one: `HTTPError` populates `error.data` by reading the body, so the old `response.clone().json()` typechecks and throws at runtime, dropping every server-supplied API notification message. Read `error.data` instead.
- **That same read had spread to ~70 sites, and their tests could not catch it.** Every `*.service.ts` wrapped it as `await error.response.json().catch(() => ({}))`, so `errorData` was ALWAYS `{}` and users always saw the generic fallback ("Failed to update room") instead of the server's reason. Fixed with `readErrorData(error)` in `src/api/client.ts`; an eslint `no-restricted-syntax` rule now bans `*.response.json()` repo-wide. The reason it survived so long: the fixtures. A fixture that sets `.data` but leaves the response stream READABLE (a hand-rolled `Object.create(HTTPError.prototype)` with `json: () => Promise.resolve(body)`, or a real `Response` that is never read) lets the buggy code pass — 29 tests asserted the exact server message and proved nothing. Build ky errors with `src/api/testSupport/httpError.ts`, which sets `data` AND consumes the stream (`void response.json().catch(...)` marks `bodyUsed` synchronously); it has its own test asserting `bodyUsed === true`. Verify any fix here by reverting the production line and confirming the tests go red AT the message assertion.
- `bun run typecheck` and `bun run test` are independent gates and vitest is the weaker one: it transpiles without type information, so code using APIs newer than the configured `lib` passes tests and fails `tsc`. Run typecheck after adding tests. (**Corrected 2026-09-15:** `tsconfig.json` is now `target`/`lib` **ES2024**, not ES2020 — `.at()`, `Object.groupBy` and `findLast` are all legal today. The gate-independence rule stands; the specific examples no longer do.)
...
- bun does not support nested `overrides` and mis-resolves self-referential `npm:` aliases — do not attempt npm/pnpm override recipes here. TypeScript is held at ~6.0 because typescript-eslint needs the TS JS API that 7.0 does not ship. **Re-verified 2026-09-04 and still blocking:** with `typescript@7.0.2` installed, `tsc --noEmit` passes with zero errors, but `@typescript-eslint/parser` *throws at module load* (`typescript-eslint does not support TS 7.0`) because `dist/index.js` reads `ts.versionMajorMinor` and hard-throws on major >= 7 — so `lint`/`lint:strict` die, and that is a CI gate. The installed parser (**8.70.0**, re-verified 2026-09-15 — its `peerDependencies.typescript` and `typescript-estree`'s `SUPPORTED_TYPESCRIPT_VERSIONS` both still read `>=4.8.4 <6.1.0`) has not moved the boundary; upstream tracking is typescript-eslint#10940, and support is aimed at TS >=7.1, not 7.0. The official side-by-side escape hatch does not apply: parser + typescript-estree make 13 bare `require("typescript")` calls with no redirection hook, so pointing them at a TS 6 copy needs nested installs that bun cannot express in package.json. Do not retry the bump until #10940 closes. TS 6.0 hard-errors on `alwaysStrict` (TS5107), and browser code needs `ReturnType<typeof setTimeout>`, not `NodeJS.Timeout`.
