...
- `customer_ledgers` — company info, `description`, `expense_type`, `amount`, `paid_amount`, `balance_due` (DB-derived), `status` ∈ {pending, partial, paid, overdue, void} (the `valid_status` CHECK allows `void`, not `cancelled` — verified 2026-07-26), `due_date`, `invoice_number`, optional `booking_id`/`guest_id`, accounting fields (`folio_type`, `transaction_type`, `post_type`, `is_reversal`, `original_transaction_id`, `void_at/by/reason`).
...
- **`create_ledger_payment`** (ledger.rs:726) — validates positive amount + non-voided ledger, inserts payment row, recomputes `paid_amount` and status: `new_total_paid ≥ total_amount` → `paid`, `> 0` → `partial`, else `pending` (ledger.rs:794).
...
- **`void_ledger`** (ledger.rs:931) — refuses if `void_at`/voided flag is already set (ledger.rs:947: `"Ledger is already voided"`); stamps void fields + status `void` (ledger.rs:961).
