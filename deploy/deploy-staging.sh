#!/usr/bin/env bash
# Deploy saliminn STAGING (staging.saliminn.my) on the same AIC VPS as prod.
#
# Usage (as root):
...
readonly APP_DIR=/opt/saliminn-staging
...
readonly CADDY_SITE_FILE=/etc/caddy/saliminn-staging.Caddyfile
...
install_release_files() {
  ...
  install -m 0644 "$RELEASE_DIR/database/patches/0007_consent_locale_zh_tw.sql" "$APP_DIR/database/patches/0007_consent_locale_zh_tw.sql"
  install -m 0644 "$RELEASE_DIR/database/patches/0008_distributed_state.sql" "$APP_DIR/database/patches/0008_distributed_state.sql"
...
BACKUP_SERVICE
  ...
  cat > /etc/systemd/system/saliminn-staging-backup.timer <<'BACKUP_TIMER'
[Unit]
...
wait_for_database_baseline() {
  ...
  while (( SECONDS < deadline )); do
    if docker exec saliminn-staging-db pg_isready -h 127.0.0.1 -U hotel_admin -d hotel_management \
      >/dev/null 2>&1; then
      baseline_revision=$(docker exec saliminn-staging-db sh -c \
      ...
      if [[ "$baseline_revision" == 1 ]]; then
        log "saliminn-staging-db final TCP server has the V1 baseline"
        return 0
      fi
    fi
    sleep 3
  done
  log "saliminn-staging-db final TCP server with V1 baseline did not become ready before the timeout"
  return 1
}
...
deploy_tag() {
  ...
  wait_for_healthy saliminn-staging-db || return 1
  wait_for_healthy saliminn-staging-backend || return 1
  wait_for_healthy saliminn-staging-frontend || return 1
  curl -fsS http://127.0.0.1:3031/health >/dev/null || return 1
  curl -fsS http://127.0.0.1:8083/health >/dev/null || return 1
}
...
backup_existing_database() {
  ...
  if docker exec saliminn-staging-db \
...
ensure_initial_admin_password() {
  ...
  current_hash=$(docker exec saliminn-staging-db \
  ...
  updated=$(docker exec -i saliminn-staging-db \
    psql -U hotel_admin -d hotel_management --tuples-only --no-align \
...
configure_caddy() {
  ...
  site_tmp=$(mktemp "$APP_DIR/.saliminn-staging.Caddyfile.XXXXXX")
  main_backup=$(mktemp "$APP_DIR/.Caddyfile.XXXXXX")
  cp -p "$CADDY_FILE" "$main_backup"
  if [[ -f "$CADDY_SITE_FILE" ]]; then
    site_backup=$(mktemp "$APP_DIR/.saliminn-staging.Caddyfile.previous.XXXXXX")
...
www.staging.saliminn.my {
    redir https://staging.saliminn.my{uri} permanent
}
CADDY

  # Bake host-local basic-auth hash into the generated site file (never from git).
  hash_file="$APP_DIR/basic-auth.hash"
  if [[ ! -s "$hash_file" ]]; then
    die "missing $hash_file — create with: caddy hash-password --plaintext '…' > $hash_file"
  fi
  HASH=$(tr -d '\n' < "$hash_file")
  sed -i "s|__BASIC_AUTH_HASH_WILL_BE_SUBSTITUTED_AT_RUNTIME__|${HASH}|g" "$site_tmp"

  install -m 0644 "$site_tmp" "$CADDY_SITE_FILE"
  if ! grep -Fqx "import $CADDY_SITE_FILE" "$CADDY_FILE"; then
    printf '\n# Hotel application (managed by /opt/saliminn-staging/deploy.sh)\nimport %s\n' \
