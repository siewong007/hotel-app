...
# /opt/saliminn-staging/backups, plus a tar.gz of the upload trees that
# pg_dump cannot cover. Off-host shipping/encryption remains a manual step
...
readonly UPLOADS_DIR=${SALIMINN_UPLOADS_DIR:-/opt/saliminn-staging/data}
...
# Same uploads coverage as the production script: the dump alone cannot
# restore eKYC images, receipts, or room-type photos. The ok status is
# written before an uploads failure so last_success keeps tracking the
# database recovery point while the error still reaches the health check.
uploads_failed=""
