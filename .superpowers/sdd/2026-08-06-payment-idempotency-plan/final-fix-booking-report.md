...
- Non-empty transaction references take a transaction-scoped advisory lock before lookup, covering the absent-row race across different bookings. The stored canonical fingerprint must match exactly; a changed amount, method, type, reference (through the idempotency-key guard), notes, requested date, another booking, or a legacy/null fingerprint returns `ApiError::Conflict`.
...
- `DATABASE_URL=... cargo test --test payment_characterization -- --nocapture`: 19 passed, 3 pre-existing ignored, 0 failed.
